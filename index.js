...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Saiu e não pagou a coca @${num.split('@')[0]}🤬`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["27996463517@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas ${prefix}stiker ou tag de imagem que foi enviado`)
					}
					break
				case 'gtts':
                ...
                case 'dono':
					...
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSArT7_KQtPf3qL_5g8NHIDsFNYUFzGNljZ5w&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME:* ZEUS\n*WPP:* wa.me/+55996463517\n NINGUEM IMPORTANTE'})
					break 
				case 'nulia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQi-yIna4_sZqH-VkdcOvUNRtwIfx0AZHnaAQ&usqp=CAU)`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME:* NULIA\n*WPP:* VAI TER NÃO VACILÃO\n *ELA PEDIU, POR ISSO ESTÁ AQUI 😳👉👈'})
					break 
				case 'data':
		            if (args.length < 1) return reply ('Sei lá, olha no seu celular, baiano 😡')
	            	break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'clearall':
					...
					reply('Todos os chats foram excluídos 😎👍')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é Você?')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('A transmissão foi um sucesso 😎👍')
					} else {
						for (let _ of anu) {
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar, provavelmente porque era privado')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responda o sticker ❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falha ao converter adesivo em imagens ❌')
